    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (result)
            {
                ...
                Console.WriteLine("1：添加订单，2：删除订单，3：修改订单，4：查询订单，5：按照金额排序，0：退出");
        ...
        {
            OrderService oo = new OrderService();
            List<Order>  list_order = oo.ShowOrder();
        ...
        public static void ShowMoneyOrder()
        {
            OrderService orderService = new OrderService();
            List<OrderDetails2> list_order = orderService.ShowOrderMoney();
            for (int i = 0; i < list_order.Count; i++)
            {
                Console.Write("编号：" + list_order[i].id + "\t\t购买者：" + list_order[i].user + "\t\t商品名称：" + list_order[i].ShopName+"\t\t商品价格："+ list_order[i].price);
                Console.WriteLine("");
            }
        }
